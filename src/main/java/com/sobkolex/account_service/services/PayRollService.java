...
public class PayRollService {
    ...
    public void addPayrolls(List<PutPayrollDTO> payrolls) {
        ...
        } else if (list.stream().anyMatch(o -> o.getSalary() < 0)) {
    ...
    public GetPayrollDTO getPayrollByPeriodAndUser(String period, User user) {
        try {
            ...
            Payroll payroll = payrollRepository.findByEmployeeAndPeriod(user.getEmail(), date);
            if (payroll == null)
                throw new NotFoundException("Payroll not found!");
            else
                return convertToGetPayrollDTO(payroll);
